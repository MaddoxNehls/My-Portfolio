...
export const MenuItem = ({
  ...
  return (
    <div onMouseEnter={() => setActive(item)} className="relative">
      <motion.p transition={{ duration: 0.3 }} className="cursor-pointer rounded-full p-2 text-foreground hover:bg-foreground hover:text-background">
        {item}
      </motion.p>
      {active !== null && (
          ...
          {active === item && (
            <div className="absolute left-1/2 top-[calc(100%_+_1rem)] -translate-x-1/2">
              <motion.div
                ...
                layoutId="active"
                className="overflow-hidden rounded-2xl border border-foreground bg-background shadow-xl dark:border-foreground dark:bg-background"
              >
  ...
  return (
    <Link href={href} className="flex space-x-2">
      ...
      <div>
        ...
        <p className="max-w-40 text-sm text-muted-foreground">{description}</p>
      </div>
    </Link>
  );
};
